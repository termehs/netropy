...
The package creates variables on three variable domains: vertex, dyad and triad variables. This is exemplified in the following.
...
These cdf's can be obtained by the following syntax
...
frq.age = data.frame(value=values,freq=as.vector(x),
                       ...
                       cum.rel.freq=as.vector(cum.prop))
```
By looking at these cdf's we can find values with which we based the categories on:
...
head(att.var)
```
Note that we for the sake of consistency, we also edit all variables such that their outcomes start from the value 0. The variable `senior` only has unique values, thus it is redundant and omitted (later we illustrate how such redundant variables can be detected using bivariate entropies).
