...
To load the data, extract each object and assign the correct names to them we run the following syntax
...
H2
```
Bivariate entropies can be used to detect redundant variables that should be omitted from the dataframe for further analysis. This occurs when the univariate entropy for a variable is equal to the bivariate entropies of pairs including that variable. 
...
redundancy(dyad.var)
```
More examples of using the function `redundancy()` is given in the vignette  [univariate_bivariate_trivariate](univariate_bivariate_trivariate.html). 
...
J$freq
```
As seen, the strongest association is between the variables `status` and `years` with joint entropy values of 0.794. We have independence (joint entropy value of 0) between two pairs of variables: `status` and ` practice`, and `status` and `gender`.
...
```{r assoc_g, eval=TRUE, fig.align='center',fig.width=14, message =FALSE}
library(ggraph)
assoc_graph(dyad.var, 0.15)
```

Given this threshold, we see isolated nodes representing independent variables. 
...
More details and examples of expected conditional entropies and prediction power  are given in the vignette  [prediction_power](prediction_power.html). 
